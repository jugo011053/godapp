...
await page.locator('[data-regenerate-day]').first().click();
...
await page.getByRole('heading', { name: 'Was soll mit deinem aktuellen Plan passieren?' }).waitFor();
await page.getByRole('button', { name: 'Plan behalten' }).click();
assert.equal(await page.getByRole('heading', { name: 'Was soll mit deinem aktuellen Plan passieren?' }).count(), 0);
