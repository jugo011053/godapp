...
const rows = Array.from({ length: 18 }, (_, index) => {
  ...
  return {
    ...
    source: 'e2e',
    code: `E2E-${index}`,
    name: `${protein} ${category} ${index}`,
    cat: category,
    kcal: category === 'breakfast' ? 430 : 620 + index,
    protein: category === 'breakfast' ? 28 : 42,
    carbs: 55,
    fat: 16,
    time: 20 + (index % 3) * 5,
    servings: 2,
...
await page.route('https://rfdtjodpjvynnavnucvu.supabase.co/rest/v1/recipe_catalog_v1**', async (route) => {
  const url = new URL(route.request().url());
  const idFilter = url.searchParams.get('id');
  const response = idFilter?.startsWith('eq.')
...
await page.addInitScript(() => {
  localStorage.setItem('preply_v8_state_v1', JSON.stringify({
    ...
    profile: {
      version: 2,
      planningMode: 'simple', goal: 'maintain', calorieTarget: 2200, proteinTarget: 130,
      dietStyle: 'omnivore', allergies: [], excludedIngredients: [], excludedRecipes: [],
      persons: 1,
...
await page.goto('http://127.0.0.1:4173/v8/', { waitUntil: 'networkidle' });
await page.getByText('18 geprüfte Rezepte geladen.').waitFor();

await page.getByRole('button', { name: /Was esse ich heute/ }).click();
await assert.equal(await page.locator('text=Drei Vorschläge für heute').count(), 1);
await assert.equal(await page.locator('.recipe-card').count(), 3);
...
const after = await page.locator('[data-shop-check]').count();
